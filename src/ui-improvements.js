...
(async function () {
  ...
  `);
    ...
    const { container: enableLootFilterToggle } = createSettingsInput({
        key: "ui-improvements:showLootMonsterFilter",
        id: "showLootMonsterFilter",
        label: "Show Loot Monster Filter",
        defaultValue: true,
        type: "checkbox",
        inputProps: { slider: true },
        onChange: () => {
            updateLootMonsterFilterVisibility();
        },
    });
    ...
    if (window.location.href.includes("/active_wave.php")) {
        ...
        // -------------- Loot Monster Name Filter + Visual Filter + Counts ----------------

        const LOOT_FILTER_KEY = "ui-improvements:lootMonsterNameFilter";
        ...
        function getSelectedLootMonsterName() {
            return normalizeMonsterName(Storage.get(LOOT_FILTER_KEY, ""));
        }

        function getLootMonsterCards(doc = document) {
            return Array.from(doc.querySelectorAll('.monster-card[data-eligible="1"]'));
        }

        function getLootMonsterCounts(doc = document) {
            ...
            getLootMonsterCards(doc).forEach((el) => {
                const name = normalizeMonsterName(el.dataset.name);
                if (!name) return;

                if (ignoreBossMobsWhenLooting && LOOTING_BLACKLIST_SET.has(name)) return;
        ...
        function getLootMonsterNames(doc = document) {
            return Array.from(getLootMonsterCounts(doc).keys());
        }

        function titleCaseMonsterName(name = "") {
            return String(name)
                .split(" ")
                .filter(Boolean)
        ...
        function updateLootMonsterVisualFilter() {
            const selected = getSelectedLootMonsterName();
            const cards = getLootMonsterCards(document);
            ...
            cards.forEach((el) => {
                const name = normalizeMonsterName(el.dataset.name);
                const shouldShow = !selected || name === selected;
            ...
            const summary = document.getElementById("lootMonsterFilterSummary");
            if (summary) {
                const label = selected ? titleCaseMonsterName(selected) : "All monsters";
                summary.textContent = `${label}: ${shown} shown`;

                if (hidden > 0) {
                    summary.textContent += `, ${hidden} hidden`;
                }
            }
        }

        function updateLootMonsterCountDisplay() {
            const counts = getLootMonsterCounts(document);
            const total = Array.from(counts.values()).reduce((sum, n) => sum + n, 0);

            const countBox = document.getElementById("lootMonsterCountBox");
            const select = document.getElementById("lootMonsterNameFilter");

            if (!countBox || !select) return;
            ...
            totalLine.style.cssText = "font-weight:700;margin-bottom:4px;";
            totalLine.textContent = `Lootable monsters: ${total}`;
            countBox.appendChild(totalLine);
            ...
            Array.from(counts.entries())
                ...
                line.type = "button";
                line.dataset.lootMonsterName = name;
                line.textContent = `${titleCaseMonsterName(name)}: ${count}`;
        ...
        background:${selected === name ? "#4b5bdc" : "#252945"};
        color:#e6e8ff;
        cursor:pointer;
        font-size:12px;
      `;
                ...
                line.addEventListener("click", () => {
                    Storage.set(LOOT_FILTER_KEY, name);
                    select.value = name;
                    updateLootMonsterCountDisplay();
                    updateLootMonsterVisualFilter();
                });
        ...
        function refreshLootMonsterFilterUI() {
            const select = document.getElementById("lootMonsterNameFilter");
            if (!select) return;
            ...
            allOpt.value = "";
            allOpt.textContent = `All monsters (${Array.from(counts.values()).reduce(
                (sum, n) => sum + n,
                0
            )})`;
            ...
            names.forEach((name) => {
                ...
                opt.value = name;
                opt.textContent = `${titleCaseMonsterName(name)} (${counts.get(name)})`;
                select.appendChild(opt);
            });

            select.value = names.includes(selected) ? selected : "";
            if (selected && !names.includes(selected)) {
                Storage.set(LOOT_FILTER_KEY, "");
            }
        ...
        function injectLootMonsterFilter() {
            if (document.getElementById("lootMonsterNameFilter")) return;

            const btnLootX = document.getElementById("btnLootX");
            const lootStatus = document.getElementById("lootStatus");
            if (!btnLootX || !lootStatus) return;

            const panel = document.createElement("div");
            panel.id = "lootMonsterFilterPanel";
            panel.style.cssText = `
    margin:10px 0;
    padding:8px 10px;
    border:1px solid #2b2d44;
  ...
  `;
            ...
            label.htmlFor = "lootMonsterNameFilter";
            label.textContent = "Loot filter:";

            const select = document.createElement("select");
            select.id = "lootMonsterNameFilter";
  ...
  `;
            ...
            summary.id = "lootMonsterFilterSummary";
            summary.style.cssText = "font-size:12px;opacity:.9;";

            /*             const countBox = document.createElement("div");
            countBox.id = "lootMonsterCountBox";
  ...
  `; */
            ...
            select.addEventListener("change", () => {
                Storage.set(LOOT_FILTER_KEY, select.value);
                updateLootMonsterCountDisplay();
                updateLootMonsterVisualFilter();

                if (lootStatus) {
                    lootStatus.textContent = select.value
                        ? `Loot filter set to: ${titleCaseMonsterName(select.value)}`
        : "Loot filter cleared.";
                }
            });
            ...
            clearBtn.addEventListener("click", () => {
                ...
                if (lootStatus) {
                    lootStatus.textContent = "Loot filter cleared.";
                }
            });

            row.append(label, select, clearBtn, summary);
            panel.append(row);

            btnLootX.insertAdjacentElement("afterend", panel);

            refreshLootMonsterFilterUI();
        ...
        function updateLootMonsterFilterVisibility() {
            const panel = document.getElementById("lootMonsterFilterPanel");

            if (!panel) return;

            const enabled = Storage.get(
                "ui-improvements:showLootMonsterFilter",
                true
            );
        ...
        function getEligibleMobs(doc = document) {
            ...
            const selectedLootMonsterName = getSelectedLootMonsterName();
            ...
            return els
                ...
                if (ignoreBossMobsWhenLooting && LOOTING_BLACKLIST_SET.has(name)) {
                    return false;
                }

                if (selectedLootMonsterName && name !== selectedLootMonsterName) {
                    return false;
                }
